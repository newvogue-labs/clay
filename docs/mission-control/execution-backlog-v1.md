...
- `E14` Exchange execution adapter — multi-venue order-plane (Architecture v2) — см. [ADR-032](../adr/032-exchange-execution-adapter-multi-venue.md)
...
## E14. Exchange Execution Adapter — Multi-Venue Order-Plane (Architecture v2)

**Цель:** обобщить execution-слой (ADR-025) до мульти-венью порта — единый доменный `ExchangeAdapter` вместо N хардкодов. E14 ≠ E13: E13 = data-plane portability / ADR-008 (market-data, httpx); E14 = execution-plane order-adapter / ADR-025 → ADR-032.

** Dependencies:** `E8`, `ADR-025`, `ADR-032`  
**Deliverable:** доменный `ExchangeAdapter` + Binance adapter + reconcile + resilience

### S-ADAPT-1: ExchangeAdapter Protocol + доменные DTO

- `ExchangeAdapter` Protocol (async, по образцу vn.py)
- Доменные DTO: frozen dataclass, `Decimal` (никаких float)
- `Environment`, `OrderSide`, `OrderType`, `TimeInForce`, `OrderState` — StrEnum
- `MarketRules` — capability-overlay per market-type + per-symbol
- Доменная error-таксономия (Retryable / Terminal / Ambiguous)
- Без сети, без ccxt

### S-ADAPT-2: BinanceExecutionAdapter

- `BinanceExecutionAdapter(ExchangeAdapter)` — ccxt driver ВНУТРИ
- Замещает `TestnetExecutionClient` из ADR-025
- `validate` / `quantize` / `place_order` / `get_order` / `cancel_order` / `get_balances`
- `Environment` выбирает endpoint (testnet / demo.binance.com / production)

### S-ADAPT-3: Идемпотентность + reconciliation

- `client_order_id` = идемпотентный ключ
- `AmbiguousExecutionError` на таймаут после отправки
- `reconcile_orders(...)` — mandatory перед retry
- Слепой retry запрещён (риск двойного исполнения)

### S-ADAPT-4: CircuitBreaker + read-only fallback

- CircuitBreaker перед венью-вызовами
- Fallback НИКОГДА не роутит `place_order`/`cancel_order` на другую венью
- Read-операции (`get_order`, `get_balances`, `reconcile`) — fallback-chain разрешён
- Выбор реализации (паттерн vs `pybreaker`) — в build_spec

### S-ADAPT-5: Второй венью (Bybit demo)

- `BybitExecutionAdapter(ExchangeAdapter)` через capability-overlay
- Доказательство переносимости: один порт, два адаптера, разные MarketRules
- Bybit demo → Binance Demo → OKX demo (Track A порядок)
