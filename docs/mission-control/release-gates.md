...
Errata 2026-08-01: добавлен G7 (денежный путь), закрывает D-45; ратифицирован живым дриллом S-MONEYPATH-2 (8/8 @ 0b90f08d).
...
- **Критерий прохода:** на тестнете (`testnet.binance.vision`) проходит полный полигон заявки — постановка → снимок → отмена, финальный список открытых заявок пуст и равен исходному (счёт возвращается в ноль); путь чтения/сверки (read/reconcile) верно разбирает три типа заявки (market / limit / stop-limit) — тип, время биржи, цена и символ в ack совпадают с ответом венью; неизвестный символ отклоняется `InvalidOrderError` до сети; каждая заявка попадает в аудит-журнал venue-truth (`~/.local/state/clay/audit.jsonl`); открытых дефектов денежного пути нет; известные ограничения записаны в долговом реестре и видимы (не молчат).
...
✅ **G7 закрыт** (2026-08-01) — денежный путь доказан живым полигоном S-MONEYPATH-2 (8/8 @ `0b90f08d`); закрывает долг **D-45** («гейты не покрывали путь заявки»).
