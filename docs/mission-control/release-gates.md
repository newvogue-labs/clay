...
- **Ring 1 — Real, минимальный лимит:** реальные деньги малым фиксированным лимитом, усиленное наблюдение. **Вход:** все `G0–G7` зелёные + `E12.5` пройден + **свежий 24ч-soak** (D-58 soak-harness, ручной веховый прогон на хосте с артефактом-вердиктом, который сверяет архитектор; CI охраняет только логику харнесса — ADR-040) + Emma GO.
