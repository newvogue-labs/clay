...
def _demo_record(pnl_pct: float | None, *, outcome_status: str = "matched", operator_action: str = "entered",
                 broker_status: str = "closed", session_id: str = "s1", recorded_at: datetime | None = None,
                 advisory_size_pct: float | None = None) -> object:
    record = type("FakeDemoRecord", (), {
        "pnl_pct": pnl_pct,
...
def _risk_checks(service, db_session):
    ...
    return {c.check_id: c for c in snapshot.preflight.checks if c.check_id.startswith("risk-limit-")}
...
def test_empty_db_all_limits_pass(db_session) -> None:
    ...
    for cid, check in checks.items():
        assert check.status == "ok", f"{cid} should be ok on empty DB, got {check.status}"
...
def test_l1_drawdown_triggers_hard_fail(mock_session, mock_open, mock_ordered, mock_window, db_session) -> None:
...
@patch("clay.db.repositories_demo.DemoRepository.list_open_positions", return_value=[])
@patch("clay.db.repositories_demo.DemoRepository.list_session_trades", return_value=[])
def test_l1_drawdown_passes_below_threshold(mock_session, mock_open, mock_ordered, mock_window, db_session) -> None:
...
@patch("clay.db.repositories_demo.DemoRepository.list_open_positions", return_value=[])
@patch("clay.db.repositories_demo.DemoRepository.list_session_trades", return_value=[])
def test_l2_cooldown_triggers_hard_fail(mock_session, mock_open, mock_ordered, mock_window, db_session) -> None:
...
@patch("clay.db.repositories_demo.DemoRepository.list_open_positions", return_value=[])
@patch("clay.db.repositories_demo.DemoRepository.list_session_trades", return_value=[])
def test_l2_cooldown_expires(mock_session, mock_open, mock_ordered, mock_window, db_session) -> None:
...
@patch("clay.db.repositories_demo.DemoRepository.list_open_positions", return_value=[])
@patch("clay.db.repositories_demo.DemoRepository.list_session_trades", return_value=[])
def test_l2_single_loss_passes(mock_session, mock_open, mock_ordered, mock_window, db_session) -> None:
...
@patch("clay.db.repositories_demo.DemoRepository.list_open_positions", return_value=[])
@patch("clay.db.repositories_demo.DemoRepository.list_session_trades", return_value=[])
def test_l2_win_breaks_streak(mock_session, mock_open, mock_ordered, mock_window, db_session) -> None:
...
@patch("clay.db.repositories_demo.DemoRepository.list_session_trades", return_value=[])
def test_l4_exposure_warns_when_exceeded(mock_session, mock_open, mock_ordered, mock_window, db_session) -> None:
    """Total exposure > 4% → warn, NOT blocks_start, rollup still pass."""
    mock_open.return_value = [
        _demo_record(pnl_pct=None, advisory_size_pct=3.0, broker_status="awaiting_result"),
        _demo_record(pnl_pct=None, advisory_size_pct=2.0, broker_status="awaiting_result"),
    ]
    service = _build_service()
    checks = _risk_checks(service, db_session)
...
@patch("clay.db.repositories_demo.DemoRepository.list_open_positions")
@patch("clay.db.repositories_demo.DemoRepository.list_session_trades", return_value=[])
def test_l4_exposure_passes_within_threshold(mock_session, mock_open, mock_ordered, mock_window, db_session) -> None:
    """Total exposure ≤ 4% → ok."""
    mock_open.return_value = [
        _demo_record(pnl_pct=None, advisory_size_pct=2.0, broker_status="awaiting_result"),
    ]
...
def test_l5_session_loss_warns(mock_session, mock_open, mock_ordered, mock_window, db_session) -> None:
    """Active session with P&L ≤ -8% → warn, NOT blocks_start."""
    service = _build_service()
    object.__setattr__(service, "_active_session", ActiveSessionRecord(
        session_id="s-lossy", current_pair_symbol="BTCUSDT",
        current_signal_id="sig-1", strategy_mode="momentum",
        started_at=datetime.now(UTC),
    ))
...
@patch("clay.db.repositories_demo.DemoRepository.list_open_positions", return_value=[])
@patch("clay.db.repositories_demo.DemoRepository.list_session_trades")
def test_l5_session_loss_passes(mock_session, mock_open, mock_ordered, mock_window, db_session) -> None:
    """Active session with small P&L → ok."""
    service = _build_service()
    object.__setattr__(service, "_active_session", ActiveSessionRecord(
        session_id="s-good", current_pair_symbol="BTCUSDT",
        current_signal_id="sig-1", strategy_mode="momentum",
        started_at=datetime.now(UTC),
    ))
    mock_session.return_value = [
...
def test_warn_checks_do_not_block_rollup(db_session) -> None:
    ...
    service = _build_service()
    snapshot = service.build_snapshot(db_session)
    warn_count = sum(1 for c in snapshot.preflight.checks if c.check_id.startswith("risk-limit-") and c.status == "warn")
    if warn_count > 0:
