"""Integration test: duplicate-intent probe late-bind wiring in bootstrap.

Verifies that ``build_services`` wires ``set_session_duplicate_intent_probe``
into ``ExecutionProofGate`` exactly when all three conditions hold:
  1. ``CLAY_EXECUTION_MODE=testnet`` + non-empty API key/secret
     (→ ``execution_client`` is built)
  2. ``CLAY_PROOF_ENFORCE_SESSION=1``
  3. ``CLAY_PROOF_DUPLICATE_INTENT_WINDOW_SECONDS > 0``

Replaces the tautological ``TestBootstrapDuplicateIntentWiring`` unit tests
...
_BASE_ENV: dict[str, str] = {
    ...
    "CLAY_PROOF_DUPLICATE_INTENT_WINDOW_SECONDS": "30",
}
...
class TestBootstrapDuplicateIntentWiring:
    ...
    def test_all_conditions_probe_bound(
        ...
        assert ec._session_duplicate_intent_probe is not None  # noqa: SLF001
    ...
    def test_enforce_session_off_probe_none(
        ...
        ec = services["execution_client"]
        assert ec is not None
        assert ec._session_duplicate_intent_probe is None  # noqa: SLF001

    def test_window_zero_probe_none(
        self, tmp_path: object, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        env = {**_BASE_ENV, "CLAY_PROOF_DUPLICATE_INTENT_WINDOW_SECONDS": "0"}
        _set_env(monkeypatch, env)
        services = build_services_for_integration(tmp_path)  # type: ignore[arg-type]
        ec = services["execution_client"]
        assert ec is not None
