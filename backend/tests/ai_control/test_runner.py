...
def test_ollama_settings_defaults() -> None:
    for k in (
        "CLAY_OLLAMA_BASE_URL",
        "CLAY_OLLAMA_NUM_CTX",
        "CLAY_OLLAMA_TIMEOUT_SECONDS",
    ):
...
def test_ollama_settings_env_override() -> None:
    keys = {
        "CLAY_OLLAMA_BASE_URL": "http://127.0.0.1:9999",
        "CLAY_OLLAMA_NUM_CTX": "4096",
    }
...
def test_ollama_client_from_settings_uses_settings_values() -> None:
    ...
    client = OllamaNativeClient.from_settings(
...
def test_litellm_client_reasoning_content_fallback() -> None:
    ...
    async def _mock_chat(content: str | None, reasoning: str | None):
        msg = LLMChatMessage(
            role="assistant", content=content or "", reasoning_content=reasoning
        )
        ...
        resp = await client.chat(
            [ChatMessage(role="user", content="hi")], model="gemma-4-31b"
        )
        return resp
